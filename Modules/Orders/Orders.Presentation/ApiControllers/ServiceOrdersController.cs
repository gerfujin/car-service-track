...
public class ServiceOrdersController : ControllerBase
{
    ...
    public async Task<ActionResult<ServiceOrderDto>> CreateServiceOrder([FromBody] ServiceOrderCreateDto dto)
    {
        ...
        {
            // Admin can create for any vehicle.
            vehicle = await _sender.Send(new GetVehicleByIdQuery(dto.VehicleId));
            if (vehicle == null)
            {
        ...
        else
        {
            ...
            var owner = await _sender.Send(new GetOwnerByAppUserIdQuery(userId));
            if (owner == null)
            {
                return BadRequest(new RestApiErrorResponse
                {
            ...
            vehicle = await _sender.Send(new GetVehicleByIdForOwnerQuery(dto.VehicleId, owner.Id));
            if (vehicle == null)
            {
                return BadRequest(new RestApiErrorResponse
                {
        ...
        var workshop = await _sender.Send(new GetWorkshopByIdQuery(dto.WorkshopId));
        if (workshop == null)
        {
            return BadRequest(new RestApiErrorResponse
            {
